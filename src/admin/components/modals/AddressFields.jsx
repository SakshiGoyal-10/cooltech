...
import { FRow, FInput, FSelect } from '../ui/Form';
...
// Drop this anywhere inside a modal form.
// `prefix` keeps field names unique if used in two modals on same page.
const AddressFields = ({ prefix = '' }) => {
  const [country, setCountry] = useState('');
  const [state,   setState]   = useState('');
  const [city,    setCity]    = useState('');

  const states = country ? Object.keys(ADDRESS_DATA[country] || {}) : [];
  const cities = (country && state) ? (ADDRESS_DATA[country]?.[state]?.cities || []) : [];
    ...
    <div className="addr-wrap">
      ...
      <div className="addr-grid-2">
        ...
        <FRow label="Area / Locality">
          <FInput placeholder="e.g. Koramangala, Sector 15…" name={`${prefix}area`} />
        </FRow>

        {/* ZIP / PIN */}
        <FRow label="ZIP / PIN Code">
          <FInput placeholder="e.g. 560034" name={`${prefix}zip`} maxLength={10} />
        </FRow>
      </div>
    </div>
  );
