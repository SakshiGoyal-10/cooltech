...
const MOCK_SO_ORDERS = [
  {
    id: 'SO-0091', customer: 'Arjun Mehta', phone: '98765 43210',
    items: [
    ...
    subtotal: 45400, gst: 8172, total: 53572,
    orderDate: 'Apr 15, 2026', deliveryDate: 'Apr 20, 2026',
    status: 'delivered', payStatus: 'paid',
    address: '12, Satellite Road, Ahmedabad',
    notes: 'Premium installation requested.',
  },
  {
    id: 'SO-0090', customer: 'Priya Sharma', phone: '91234 56789',
    items: [
      { name: 'Window AC 1T', qty: 1, rate: 18000, total: 18000 },
    ],
    subtotal: 18000, gst: 3240, total: 21240,
    orderDate: 'Apr 14, 2026', deliveryDate: 'Apr 18, 2026',
    status: 'shipped', payStatus: 'paid',
    address: '45, Navrangpura, Ahmedabad',
    notes: '',
  },
  {
    id: 'SO-0089', customer: 'Rohan Constructions', phone: '90000 11223',
    items: [
    ...
    subtotal: 160000, gst: 28800, total: 188800,
    orderDate: 'Apr 12, 2026', deliveryDate: 'Apr 16, 2026',
    status: 'delivered', payStatus: 'paid',
    address: 'Plot 8, GIDC Estate, Vatva',
    notes: 'Bulk order — dedicated technician assigned.',
  },
  {
    id: 'SO-0088', customer: 'Sneha Patel', phone: '87654 32109',
    items: [
      { name: 'Inverter AC 1.5T',  qty: 1, rate: 38000, total: 38000 },
      { name: 'Extended Warranty', qty: 1, rate: 4500,  total: 4500  },
    ],
    subtotal: 42500, gst: 7650, total: 50150,
    orderDate: 'Apr 10, 2026', deliveryDate: 'Apr 14, 2026',
    status: 'processing', payStatus: 'pending',
    address: '7, Paldi Cross Road, Ahmedabad',
    notes: 'Preferred delivery after 6 PM.',
  },
  {
    id: 'SO-0087', customer: 'Vikram HVAC Works', phone: '99887 76655',
    items: [
      { name: 'Duct AC 3T',     qty: 1, rate: 95000, total: 95000 },
      { name: 'Copper Pipe 5m', qty: 4, rate: 900,   total: 3600  },
    ],
    subtotal: 98600, gst: 17748, total: 116348,
    orderDate: 'Apr 9, 2026', deliveryDate: 'Apr 13, 2026',
    status: 'processing', payStatus: 'pending',
    address: '22, Iscon Ambli Road, Bopal',
    notes: '',
  },
  {
    id: 'SO-0086', customer: 'Deepa Iyer', phone: '76543 21098',
    items: [
      { name: 'Portable AC 1T', qty: 1, rate: 22000, total: 22000 },
    ],
    subtotal: 22000, gst: 3960, total: 25960,
    orderDate: 'Apr 7, 2026', deliveryDate: 'Apr 11, 2026',
    status: 'delivered', payStatus: 'paid',
...
export const SO_STATUS = {
  processing: { label: 'Processing', color: '#185FA5', bg: '#E6F1FB' },
  shipped:    { label: 'Shipped',    color: '#534AB7', bg: '#EEEDFE' },
  delivered:  { label: 'Delivered',  color: '#3B6D11', bg: '#EAF3DE' },
  cancelled:  { label: 'Cancelled',  color: '#A32D2D', bg: '#FCEBEB' },
};

// ─── Column configs for export ────────────────────────────────────────────────
const PO_COLUMNS = [
  { label: 'PO ID',      key: 'id',          width: 12, tdStyle: { fontFamily: 'monospace' } },
  { label: 'Supplier',   key: 'supplier',     width: 22, tdStyle: { fontWeight: 600 } },
  { label: 'Items',      key: 'itemCount',    width: 8  },
  { label: 'Subtotal',   key: 'subtotal',     width: 14, format: v => `₹${(v ?? 0).toLocaleString()}`, tdStyle: { fontFamily: 'monospace' } },
  { label: 'GST',        key: 'gst',          width: 12, format: v => `₹${(v ?? 0).toLocaleString()}`, tdStyle: { fontFamily: 'monospace' } },
  { label: 'Total',      key: 'total',        width: 14, format: v => `₹${(v ?? 0).toLocaleString()}`, tdStyle: { fontFamily: 'monospace', fontWeight: 800 } },
  { label: 'Order Date', key: 'orderDate',    width: 12 },
  { label: 'Expected',   key: 'expectedDate', width: 12 },
  { label: 'Status',     key: 'status',       width: 10, format: v => PO_STATUS[v]?.label ?? v },
  { label: 'Payment',    key: 'payStatus',    width: 10, format: v => PAY_STATUS[v]?.label ?? v },
];

const SO_COLUMNS = [
  { label: 'Order ID',   key: 'id',           width: 12, tdStyle: { fontFamily: 'monospace' } },
  ...
  { label: 'Total',      key: 'total',         width: 14, format: v => `₹${(v ?? 0).toLocaleString()}`, tdStyle: { fontFamily: 'monospace', fontWeight: 800 } },
  { label: 'Order Date', key: 'orderDate',     width: 12 },
  { label: 'Delivery',   key: 'deliveryDate',  width: 12 },
  { label: 'Status',     key: 'status',        width: 10, format: v => SO_STATUS[v]?.label ?? v },
  { label: 'Payment',    key: 'payStatus',     width: 10, format: v => PAY_STATUS[v]?.label ?? v },
];
...
const TabBar = ({ activeTab, onChange }) => {
  const tabs = [
    { key: 'admin',    label: 'Admin / Supplier View' },
...
// ─── PO Detail (Admin) ────────────────────────────────────────────────────────
const PODetail = ({ po, suppliers, onBack, onSave, onDelete, openModal, initialEditMode }) => {
  const fields = [
    { key: 'supplier' }, { key: 'orderDate' }, { key: 'expectedDate' },
    { key: 'status' },   { key: 'payStatus' }, { key: 'notes' },
  ];
    ...
    <div className="po-sidebar">
      <div className="po-sidebar-card">
        ...
        {['draft', 'ordered', 'received'].map(s => {
          ...
          return (
            <button
              key={s}
              className={`btn po-status-btn${po.status === s ? ' po-status-btn--active' : ''}`}
              style={po.status === s ? { background: m.bg, color: m.color, borderColor: m.color + '40' } : {}}
              onClick={() => openModal('report', { title: `Update to ${m.label}`, format: 'Update' })}
            >
              {po.status === s ? '● ' : '○ '}{m.label}
            </button>
          );
      ...
      {supplier && (
        <div className="po-sidebar-card">
          ...
          {[['Contact', supplier.contact], ['Phone', supplier.phone], ['Terms', supplier.paymentTerms]].map(([k, v]) => (
            <div key={k} className="po-detail-row">
            ...
            <div className={`po-main-card${editMode ? ' po-main-card--edit' : ''}`}>
              <div className="po-main-hdr">
                <div className="po-main-hdr-left">
                  ...
                  {editMode ? (
                    <input className="form-input po-supplier-input" value={val('supplier')} onChange={setK('supplier')} />
                  ) : (
                    <div className="po-supplier-name">{po.supplier}</div>
                  )}
                  <div className="po-dates">
                    {editMode ? (
                      <div className="po-dates-edit">
                      ...
                      <span>Order: {po.orderDate} · Expected: {po.expectedDate}</span>
                    )}
                  </div>
                </div>
                  ...
                  <thead>
                    <tr>
                      {['Item', 'Qty', 'Rate', 'Total'].map(h => (
                        <th key={h} className={h === 'Item' ? '' : 'po-th-center'}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
              ...
              <div className="po-totals-wrap">
                <div className="po-totals">
                  {[['Subtotal', po.subtotal], ['GST 18%', po.gst]].map(([k, v]) => (
                    <div key={k} className="po-totals-row">
              ...
              {!editMode && (
                <div className="po-actions">
                  <button className="btn btn-primary po-action-main" onClick={() => openModal('report', { title: `Mark ${po.id} Received`, format: 'Update' })}>
                    📦 Mark Received
                  </button>
                  <button className="btn btn-success po-action-paid" onClick={() => openModal('report', { title: `Mark ${po.id} Paid`, format: 'Update' })}>
                    ✓ Mark Paid
                  </button>
                </div>
              )}
...
// ─── SO Detail (Customer) ─────────────────────────────────────────────────────
const SODetail = ({ so, onBack, onSave, onDelete, openModal, initialEditMode }) => {
  const fields = [
    { key: 'customer' }, { key: 'phone' },        { key: 'address' },
    { key: 'orderDate' }, { key: 'deliveryDate' },
    { key: 'status' },   { key: 'payStatus' },    { key: 'notes' },
  ];
  ...
  const sidebar = (
    <div className="po-sidebar">
      <div className="po-sidebar-card">
        ...
        {['processing', 'shipped', 'delivered', 'cancelled'].map(s => {
          ...
          return (
            <button
              key={s}
              className={`btn po-status-btn${so.status === s ? ' po-status-btn--active' : ''}`}
              style={so.status === s ? { background: m.bg, color: m.color, borderColor: m.color + '40' } : {}}
              onClick={() => openModal('report', { title: `Update to ${m.label}`, format: 'Update' })}
            >
              {so.status === s ? '● ' : '○ '}{m.label}
            </button>
      ...
      <div className="po-sidebar-card">
        ...
        {[['Phone', so.phone], ['Address', so.address]].map(([k, v]) => (
          v ? (
            <div key={k} className="po-detail-row">
              <span className="po-detail-key">{k}</span>
              <span className="po-detail-val">{v}</span>
            ...
            <div className={`po-main-card${editMode ? ' po-main-card--edit' : ''}`}>
              <div className="po-main-hdr">
                <div className="po-main-hdr-left">
                  ...
                  {editMode ? (
                    <input className="form-input po-supplier-input" value={val('customer')} onChange={setK('customer')} />
                  ) : (
                    <div className="po-supplier-name">{so.customer}</div>
                  )}
                  <div className="po-dates">
                    {editMode ? (
                      <div className="po-dates-edit">
                      ...
                      <span>Order: {so.orderDate} · Delivery: {so.deliveryDate}</span>
                    )}
                  </div>
                </div>
                <div className="po-main-hdr-right">
              ...
              <div className="po-totals-wrap">
                <div className="po-totals">
                  {[['Subtotal', so.subtotal], ['GST 18%', so.gst]].map(([k, v]) => (
                    <div key={k} className="po-totals-row">
                      <span className="po-totals-key">{k}</span>
              ...
              {editMode ? (
                ...
                so.notes && <div className="po-notes">📌 {so.notes}</div>
              )}

              {!editMode && (
                <div className="po-actions">
                  <button className="btn btn-primary po-action-main" onClick={() => openModal('report', { title: `Mark ${so.id} Shipped`, format: 'Update' })}>
                    🚚 Mark Shipped
                  </button>
                  <button className="btn btn-success po-action-paid" onClick={() => openModal('report', { title: `Mark ${so.id} Paid`, format: 'Update' })}>
                    ✓ Mark Paid
                  </button>
                </div>
              )}
...
const AdminPOList = ({ openModal }) => {
  ...
  const normalisePO = (p) => {
    ...
    return {
      ...
      supplier:     typeof p.supplier === 'object'
                      ? (p.supplier?.name ?? '')
  ...
  useEffect(() => {
    purchaseApi.list({ limit: 200 })
      .then(r => setOrders((r.data ?? []).map(normalisePO)))
      .catch(() => {});
  ...
  const enrichedOrders = orders.map(o => ({ ...o, itemCount: (o.items ?? []).length }));

  const { q, setQ, activeFilters, setFilter, filtered: searchFiltered } =
    useTableSearch(enrichedOrders, ['id', 'supplier'], { status: '', payStatus: '' });

  const filtered = searchFiltered
    .filter(r => !activeFilters.status    || r.status    === activeFilters.status)
    .filter(r => !activeFilters.payStatus || r.payStatus === activeFilters.payStatus);
  ...
  const handleDelete = (id)      => { setOrders(prev => prev.filter(p => p.id !== id)); setOpen(null); };
      ...
      <div className="card">
        <div style={{ padding: '12px 16px', borderBottom: `1px solid ${COLORS.border}`, display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <FilterSelect
            ...
            options={Object.keys(PO_STATUS)}
            allLabel="All Statuses"
            labelMap={Object.fromEntries(Object.entries(PO_STATUS).map(([k, v]) => [k, v.label]))}
          />
          <FilterSelect
            value={activeFilters.payStatus}
            onChange={val => setFilter('payStatus', val)}
            options={Object.keys(PAY_STATUS)}
            allLabel="All Payment"
            labelMap={Object.fromEntries(Object.entries(PAY_STATUS).map(([k, v]) => [k, v.label]))}
          />
          ...
          <table className="data-table">
            ...
            <tbody>
              ...
              {paginated.map((p, i) => (
                <tr
                  key={p._id ?? p.id ?? i}
                  className={`data-table tbody tr${i % 2 !== 0 ? ' row-alt' : ''}`}
                  onClick={() => { setInitialEdit(false); setOpen(p.id); }}
                >
                  ...
                  <td onClick={e => e.stopPropagation()}>
                    <ActionDropdown
                      onView={()   => { setInitialEdit(false); setOpen(p.id); }}
                      onEdit={()   => { setInitialEdit(true);  setOpen(p.id); }}
...
// ─── Customer SO List ─────────────────────────────────────────────────────────
const CustomerSOList = ({ openModal }) => {
  const [open,         setOpen]         = useState(null);
  ...
  const normaliseSO = (o) => {
    ...
    return {
      ...
      customer:     typeof o.customer === 'object'
                      ? (o.customer?.name ?? '')
  ...
  const { q, setQ, activeFilters, setFilter, filtered: searchFiltered } =
    useTableSearch(enrichedOrders, ['id', 'customer'], { status: '', payStatus: '' });
  ...
  const handleDelete = (id)      => { setOrders(prev => prev.filter(o => o.id !== id)); setOpen(null); };
  const handleBack   = ()        => { setOpen(null); setInitialEdit(false); };
  ...
  return (
    <>
      ...
      <div className="card">
        <div style={{ padding: '12px 16px', borderBottom: `1px solid ${COLORS.border}`, display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <FilterSelect
            value={activeFilters.status}
            onChange={val => setFilter('status', val)}
            options={Object.keys(SO_STATUS)}
            allLabel="All Statuses"
            labelMap={Object.fromEntries(Object.entries(SO_STATUS).map(([k, v]) => [k, v.label]))}
          />
          <FilterSelect
            value={activeFilters.payStatus}
            onChange={val => setFilter('payStatus', val)}
            ...
            <tbody>
              ...
              {paginated.map((o, i) => (
                <tr
                  key={o._id ?? o.id ?? i}
                  className={`data-table tbody tr${i % 2 !== 0 ? ' row-alt' : ''}`}
                  onClick={() => { setInitialEdit(false); setOpen(o.id); }}
                >
                  ...
                  <td onClick={e => e.stopPropagation()}>
                    <ActionDropdown
                      onView={()   => { setInitialEdit(false); setOpen(o.id); }}
                      onEdit={()   => { setInitialEdit(true);  setOpen(o.id); }}
      ...
      <DeleteConfirmModal
        ...
        onConfirm={() => { handleDelete(deleteTarget); setDeleteTarget(null); }}
        onCancel={() => setDeleteTarget(null)}
        message="This customer order will be permanently removed."
      />
    </>
  );
};
...
// ─── Root Page ────────────────────────────────────────────────────────────────
const PurchaseOrdersPage = ({ openModal }) => {
  ...
  return (
    <div className="page-body">
      ...
      {activeTab === 'admin'
        ? <AdminPOList    openModal={openModal} />
        : <CustomerSOList openModal={openModal} />
      }
    </div>
  );
};
