...
const kanbanCols = [
  ...
  { id: 'assigned',    label: 'Assigned',     color: '#3B82F6', bg: '#EFF6FF' },
  { id: 'in_progress', label: 'In Progress',  color: '#F97316', bg: '#FFF7ED' },
  { id: 'completed',   label: 'Completed',    color: '#22C55E', bg: '#F0FDF4' },
  { id: 'invoiced',    label: 'Invoiced',     color: '#8B5CF6', bg: '#F5F3FF' },
  { id: 'cancelled',   label: 'Cancelled',    color: '#EF4444', bg: '#FFF1F2' },
];

// ─── KanbanCard ───────────────────────────────────────────────────────────────
const KanbanCard = ({ job, onDragStart, onDragEnd, isDragging, onEdit, onDelete }) => {
  const pr  = priorityColor[job.priority] || '#6B7280';
  const jid = job._id || job.id;

  // Format scheduled date nicely
  const dateStr = job.scheduledDate
    ? new Date(job.scheduledDate).toLocaleDateString('en-IN', { day:'2-digit', month:'short' })
    : '';

  return (
    <div
      className={`kb-card${isDragging ? ' kb-card--dragging' : ''}`}
      style={{ borderLeftColor: pr }}
      draggable
      onDragStart={(e) => onDragStart(e, jid)}
      onDragEnd={onDragEnd}
    >
      <div className="kb-card-top">
        <span className="kb-card-id">{job.jobId || jid}</span>
        <span className="kb-card-dot" style={{ background: pr }} />
      </div>
      ...
      <div className="kb-card-actions">
        <button
          className="kb-card-btn kb-card-btn--edit"
          onClick={(e) => { e.stopPropagation(); onEdit(job); }}
          title="Edit"
        >✎</button>
        <button
          className="kb-card-btn kb-card-btn--del"
...
const EMPTY_FORM = {
  customerName:  '',
  customer:      '',       // ObjectId — required by model
  issue:         '',
  type:          'Service',
  priority:      'normal',
  status:        'new',
...
const JobModal = ({ job, onClose, onSave, technicians, customers }) => {
      ...
      <div className="modal-box" style={{ maxWidth: 580 }} onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-body">
          ...
          <div className="form-row">
            ...
            <input className="form-input" name="issue" value={form.issue} onChange={handle}
          ...
          <div className="grid-form-2" style={{ marginBottom: 14 }}>
            <div>
              ...
              <input className="form-input" type="date" name="scheduledDate"
                     value={form.scheduledDate} onChange={handle} />
            </div>
            <div>
              <label className="form-label">Scheduled Time</label>
              <input className="form-input" type="time" name="scheduledTime"
          ...
          <div className="form-row">
            ...
            <input className="form-input" name="ac" value={form.ac} onChange={handle}
          ...
          <div className="form-row">
            ...
            <textarea className="form-textarea" name="remarks" value={form.remarks}
                      onChange={handle} rows={2} placeholder="Additional notes..." />
          </div>
...
// ─── KanbanPage ───────────────────────────────────────────────────────────────
const KanbanPage = ({ openModal: _openModal }) => {
  ...
  const fetchAll = async () => {
    ...
    setError('');
    try {
      const [jobsData, techData, custData] = await Promise.all([
        jobsApi.list(),
        techsApi.list(),
        customersApi.list(),
      ]);
  ...
  const handleDragEnd = () => {
    setDraggingId(null); setOverCol(null); setOverCard(null);
  };
  ...
  const handleDrop = (e, colId) => {
    ...
    setJobs(prev => {
      const updated = prev.map(j =>
        (j._id || j.id) === draggingId ? { ...j, status: colId } : j
      );
      if (overCard && overCard !== draggingId) {
        ...
        if (targetIdx !== -1) { rest.splice(targetIdx, 0, dragged); return rest; }
      }
      return updated;
    });
  ...
  const handleColDragLeave = (e) => {
    if (!e.currentTarget.contains(e.relatedTarget)) {
      setOverCol(null); setOverCard(null);
    }
  };
    ...
    <div className="page-body">
      ...
      <div className="kb-header">
        ...
        <div className="kb-header-actions">
          <select className="form-select kb-filter-select" value={filter}
      ...
      <div className="kb-legend">
        {Object.entries(priorityColor).map(([k, c]) => (
          <div key={k} className="kb-legend-item">
            <span className="kb-legend-dot" style={{ background: c }} />
      ...
      {error && (
        <div className="kb-state-msg kb-state-msg--error">
          {error}
          ...
          {kanbanCols.map(col => {
            ...
            return (
              <div
                ...
                style={{ borderColor: col.color + '33', background: col.bg }}
                onDragOver={e  => handleColDragOver(e, col.id)}
                onDragLeave={handleColDragLeave}
                onDrop={e => handleDrop(e, col.id)}
              >
                <div className="kb-col-hdr">
                  <div className="kb-col-label" style={{ color: col.color }}>{col.label}</div>
                  <div className="kb-col-count" style={{ background: col.color }}>{colJobs.length}</div>
                </div>

                {isOver && draggingId && colJobs.length === 0 && (
                  <div className="kb-drop-indicator" style={{ borderColor: col.color }} />
                )}

                <div className="kb-cards">
                  {colJobs.length === 0 && !isOver
                    ? <div className="kb-empty">No jobs</div>
                    : colJobs.map(j => {
                        const jid = j._id || j.id;
                        return (
                          <div
                            key={jid}
                            className={`kb-card-wrap${overCard === jid && draggingId !== jid ? ' kb-card-wrap--over' : ''}`}
                            onDragOver={e => handleCardDragOver(e, jid)}
                          >
                            {overCard === jid && draggingId !== jid && (
                              <div className="kb-drop-line" style={{ background: col.color }} />
                            )}
                            <KanbanCard
                              job={j}
                              isDragging={draggingId === jid}
      ...
      {modal && (
        <JobModal
          ...
          onSave={handleCreate.length === 1
            ? (payload, id) => id ? handleUpdate(payload, id) : handleCreate(payload)
            : (payload, id) => id ? handleUpdate(payload, id) : handleCreate(payload)
          }
