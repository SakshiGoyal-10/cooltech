...
import { addToDeleted } from '../../store/deletedStore';   // ← ADD
...
// ─── Normalise raw invoice doc from API ───────────────────────────────────────
const normaliseInvoice = (inv) => {
  ...
  // ── Safely unwrap customer (may be populated object or plain string) ──
  const customer =
  ...
  // ── Safely unwrap job (may be populated object or plain string/ID) ──
  const job =
...
const InvoiceTemplate = ({ invoice, editMode, editData, setEditData }) => {
  ...
  const [rows, setRows] = useState([
    { sr: 1,  desc: 'AC Service / Repair Labour', qty: 1, rate: Math.round(invoice.amount * 0.6) },
    { sr: 2,  desc: 'Parts & Consumables',        qty: 1, rate: Math.round(invoice.amount * 0.4) },
    { sr: '', desc: '', qty: '', rate: '' },
    { sr: '', desc: '', qty: '', rate: '' },
    { sr: '', desc: '', qty: '', rate: '' },
    { sr: '', desc: '', qty: '', rate: '' },
  ...
  return (
    <div style={{ fontFamily: 'Arial, sans-serif', fontSize: 12, color: '#000', background: '#fff', width: '100%' }}>
        ...
        <tbody>
          <tr>
            <td style={{ padding: '6px 0', fontWeight: 700, fontSize: 12, width: '50%' }}>
              {'Invoice #: '}
              {editMode
                ? <input value={val('id')} onChange={setK('id')}
                    style={{ ...iS({ display:'inline-block', width:110, fontWeight:700, fontSize:13 }) }} />
                : <strong>{invoice.id}</strong>}
            </td>
            <td style={{ padding: '6px 0', fontWeight: 700, fontSize: 12, textAlign: 'right' }}>
              {'Invoice Date: - '}
              {editMode
                ? <input value={val('date')} onChange={setK('date')}
                    style={{ ...iS({ display:'inline-block', width:130, fontSize:12 }) }} />
      ...
      <div style={{ textAlign: 'center', marginBottom: 8 }}>
        ...
        {editMode
          ? <input value={val('subject') || ''} onChange={setK('subject')}
              placeholder="e.g. AC Servicing at Sharma Residency"
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', border: B }}>
        <tbody>
          ...
          {[
            ['Company Name',   CO.name,    'customer',     'e.g. Sharma Residency'],
            ['Address',        CO.address, 'billToAddress','Full billing address'],
            ['Contact Person', CO.contact, 'billToContact','Mr. / Ms. Name'],
            ['Phone No',       CO.phone,   'billToPhone',  '+91 XXXXX XXXXX'],
            ['Email',          CO.email,   'billToEmail',  'client@example.com'],
          ].map(([label, fromVal, toKey, ph]) => (
            <tr key={label}>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', height: '100%' }}>
                <tbody>
                  {[
                    ...
                    <tr key={label} style={{ borderBottom: BL }}>
                      <td style={{ padding: '8px 14px', fontWeight: 700, fontSize: 11, letterSpacing: '.02em' }}>{label}</td>
                      <td style={{ padding: '8px 14px', fontSize: 12, textAlign: 'right', fontFamily: FONTS.mono, fontWeight: 600 }}>{value}</td>
                    </tr>
                  ))}
                  <tr>
        ...
        <tbody>
          ...
          <tr>
            <td style={{ padding: '5px 12px', fontSize: 12 }}>
              * If you have any questions about this invoice, feel free to contact us.
            </td>
          </tr>
        </tbody>
      </table>
    ...
    <>
          ...
          const sidebar = (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              <div style={{ background: COLORS.white, borderRadius: 14, border: `1px solid ${COLORS.border}`, padding: '14px 16px', boxShadow: '0 1px 4px rgba(0,0,0,.04)' }}>
                ...
                {[['Subtotal', `₹${invoice.amount?.toLocaleString()}`], ['GST @ 18%', `₹${invoice.tax?.toLocaleString()}`]].map(([k, v]) => (
      ...
      <PDFPreview
        ...
        data={{
          ...
          gst:      invoice.tax,
          items: [
            { desc: 'AC Service / Repair Labour', qty: 1, rate: Math.round(invoice.amount * 0.6) },
            { desc: 'Parts & Consumables',        qty: 1, rate: Math.round(invoice.amount * 0.4) },
          ],
        }}
      />
    </>
...
const InvoicesPage = ({ openModal }) => {
  ...
  // ── Fetch from API on mount + window focus ────────────────────────────────
  ...
  // ── Derive invoice for detail view using _id ──────────────────────────────
    ...
    <div className="fi" style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ background: COLORS.white, borderRadius: 14, border: `1px solid ${COLORS.border}`, boxShadow: '0 1px 4px rgba(0,0,0,.05)', overflow: 'clip' }}>
        <div style={{ padding: '12px 18px', borderBottom: `1px solid ${COLORS.border}`, display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <FilterSelect value={activeFilters.status} onChange={val => setFilter('status', val)} options={['paid','pending','overdue']} allLabel="All Statuses" />
            ...
            <tbody>
              {paginated.map((inv, i) => (
                <tr key={inv._id}
                  ...
                  <td style={{ padding: '12px 14px' }}><span style={{ fontFamily: FONTS.mono, fontSize: 12, fontWeight: 600, color: COLORS.brand }}>{inv.id}</span></td>
                  <td style={{ padding: '12px 14px' }}><span style={{ fontFamily: FONTS.mono, fontSize: 11, color: COLORS.muted }}>{inv.job}</span></td>
                  <td style={{ padding: '12px 14px', fontSize: 13, fontWeight: 700, color: COLORS.h1 }}>{inv.customer}</td>
                  <td style={{ padding: '12px 14px' }}><span style={{ fontFamily: FONTS.mono, fontSize: 13, color: COLORS.h2 }}>₹{inv.amount.toLocaleString()}</span></td>
                  <td style={{ padding: '12px 14px' }}><span style={{ fontFamily: FONTS.mono, fontSize: 12, color: COLORS.muted }}>₹{inv.tax.toLocaleString()}</span></td>
                  <td style={{ padding: '12px 14px' }}><span style={{ fontFamily: FONTS.mono, fontSize: 14, fontWeight: 800, color: COLORS.h1 }}>₹{inv.total.toLocaleString()}</span></td>
                  <td style={{ padding: '12px 14px', fontSize: 12, color: COLORS.muted }}>{inv.date}</td>
                  <td style={{ padding: '12px 14px' }}><span style={{ fontSize: 12, fontWeight: 600, color: inv.status === 'overdue' ? '#DC2626' : COLORS.muted }}>{inv.due}</span></td>
        ...
        <div style={{ padding: '12px 18px', borderTop: `1px solid ${COLORS.border}`, background: '#F9FAFB', display: 'flex', gap: 32, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <span style={{ fontSize: 12, color: COLORS.muted }}>Taxable: <strong style={{ color: COLORS.h2 }}>₹{invoices.reduce((s, i) => s + i.amount, 0).toLocaleString()}</strong></span>
          <span style={{ fontSize: 12, color: COLORS.muted }}>CGST 9%: <strong style={{ color: COLORS.h2 }}>₹{Math.round(totalGST / 2).toLocaleString()}</strong></span>
          <span style={{ fontSize: 12, color: COLORS.muted }}>SGST 9%: <strong style={{ color: COLORS.h2 }}>₹{Math.round(totalGST / 2).toLocaleString()}</strong></span>
          <span style={{ fontSize: 13, fontWeight: 800, color: COLORS.brand, marginLeft: 'auto' }}>Total GST: ₹{totalGST.toLocaleString()}</span>
        </div>
